...
import { useConnectivityPoint, useUpdateConnectivityPoint } from '../hooks/useConnectivityPoints';
...
interface EditConnectivityPointModalProps {
  pointId: string | null;
  onClose: () => void;
}

export function EditConnectivityPointModal({ pointId, onClose }: EditConnectivityPointModalProps) {
  const push = useToastStore((s) => s.push);
  const open = !!pointId;
  const { data: detail, isLoading: isLoadingDetail } = useConnectivityPoint(pointId);
  const { mutate, isPending, error } = useUpdateConnectivityPoint();
  ...
  useEffect(() => {
    ...
    setOffersEsim(detail.offers_esim);
  ...
  function handleSubmit(e: FormEvent) {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          push({ variant: 'success', message: 'Point de connectivité mis à jour avec succès' });
          onClose();
        },
  ...
  return (
    <Modal open={open} onClose={onClose} title="Modifier le point de connectivité">
      {isLoadingDetail && (
