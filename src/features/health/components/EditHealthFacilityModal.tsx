...
import { useHealthFacility, useUpdateHealthFacility } from '../hooks/useHealthFacilities';
import { HEALTH_FACILITY_TYPES, HEALTH_FACILITY_TYPE_LABELS, type HealthFacilityType } from '../types';
import styles from '../../../shared/ui/formLayout.module.css';

interface EditHealthFacilityModalProps {
  facilityId: string | null;
  onClose: () => void;
}

export function EditHealthFacilityModal({ facilityId, onClose }: EditHealthFacilityModalProps) {
  const push = useToastStore((s) => s.push);
  const open = !!facilityId;
  const { data: detail, isLoading: isLoadingDetail } = useHealthFacility(facilityId);
  const { mutate, isPending, error } = useUpdateHealthFacility();

  const [name, setName] = useState('');
  const [type, setType] = useState<HealthFacilityType>('pharmacie');
  ...
  function handleSubmit(e: FormEvent) {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          push({ variant: 'success', message: 'Établissement de santé mis à jour avec succès' });
          onClose();
        },
  ...
  return (
    <Modal open={open} onClose={onClose} title="Modifier l'établissement de santé">
      {isLoadingDetail && (
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.row}>
            <div className={styles.field}>
              ...
              >
                {HEALTH_FACILITY_TYPES.map((t) => (
                  <option key={t} value={t}>
                    {HEALTH_FACILITY_TYPE_LABELS[t]}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className={styles.row}>
            ...
            <label className={styles.checkboxRow}>
              <input type="checkbox" checked={isOnDuty} onChange={(e) => setIsOnDuty(e.target.checked)} />
              De garde
            </label>
          </div>
