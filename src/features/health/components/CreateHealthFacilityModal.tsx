...
import { useCreateHealthFacility } from '../hooks/useHealthFacilities';
...
interface CreateHealthFacilityModalProps {
  open: boolean;
  onClose: () => void;
}

export function CreateHealthFacilityModal({ open, onClose }: CreateHealthFacilityModalProps) {
  const push = useToastStore((s) => s.push);
  const { mutate, isPending, error } = useCreateHealthFacility();
  ...
  function handleSubmit(e: FormEvent) {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          push({ variant: 'success', message: 'Établissement de santé créé avec succès' });
          resetAndClose();
        },
  ...
  return (
    <Modal open={open} onClose={resetAndClose} title="Nouvel établissement de santé">
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <Button type="submit" fullWidth disabled={isPending}>
          {isPending ? <Spinner size={18} /> : "Créer l'établissement"}
        </Button>
      </form>
    </Modal>
  );
