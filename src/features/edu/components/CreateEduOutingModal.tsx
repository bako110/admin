...
import { useCreateEduOuting } from '../hooks/useEduOutings';
...
interface CreateEduOutingModalProps {
  open: boolean;
  onClose: () => void;
}

export function CreateEduOutingModal({ open, onClose }: CreateEduOutingModalProps) {
  const push = useToastStore((s) => s.push);
  const { mutate, isPending, error } = useCreateEduOuting();
  ...
  function handleSubmit(e: FormEvent) {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          push({ variant: 'success', message: 'Sortie éducative créée avec succès' });
          resetAndClose();
        },
  ...
  return (
    <Modal open={open} onClose={resetAndClose} title="Nouvelle sortie éducative">
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.row}>
          <div className={styles.field}>
            ...
            <select id="type" className={styles.select} value={type} onChange={(e) => setType(e.target.value as EduOutingType)}>
        ...
        <Button type="submit" fullWidth disabled={isPending}>
          {isPending ? <Spinner size={18} /> : 'Créer la sortie'}
        </Button>
      </form>
    </Modal>
  );
