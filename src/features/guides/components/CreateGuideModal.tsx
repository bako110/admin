...
export function CreateGuideModal({ open, onClose }: CreateGuideModalProps) {
  const push = useToastStore((s) => s.push);
  const { mutate, isPending, error } = useCreateGuide();
  ...
  function resetAndClose() {
    ...
    setLanguages('');
    setSpecialties('');
    setRegionsCovered(BURKINA_REGIONS[0]);
    setDailyRate('');
    setHourlyRate('');
    setPhotos([]);
    onClose();
  ...
  function handleSubmit(e: FormEvent) {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          push({ variant: 'success', message: 'Guide créé avec succès' });
          resetAndClose();
        },
  ...
  return (
    <Modal open={open} onClose={resetAndClose} title="Nouveau guide">
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.field}>
          ...
          <textarea id="bio" className={styles.textarea} rows={4} value={bio} onChange={(e) => setBio(e.target.value)} />
        </div>
        ...
        <Button type="submit" fullWidth disabled={isPending}>
          {isPending ? <Spinner size={18} /> : 'Créer le guide'}
        </Button>
      </form>
    </Modal>
  );
