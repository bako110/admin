...
import { EVENT_CATEGORY_LABELS, type EventSummary } from '../types';
...
export function EventsPage() {
  ...
  const { data, isLoading, isError, refetch } = useEvents(page, 20);
  ...
  function handleConfirmDelete() {
    ...
    deleteEvent(deleteTarget.id, {
      onSuccess: () => {
        push({ variant: 'success', message: 'Événement supprimé avec succès' });
  ...
  const columns: DataTableColumn<EventSummary>[] = [
    {
      key: 'photo',
      header: '',
      width: '56px',
    ...
    { key: 'title', header: 'Titre', render: (row) => <span className={styles.name}>{row.title}</span> },
    {
      key: 'category',
      header: 'Catégorie',
      render: (row) => <Badge tone="brand">{EVENT_CATEGORY_LABELS[row.category]}</Badge>,
    },
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Festivals, concerts, foires et autres événements.</p>
        </div>
      ...
      {!isLoading && !isError && data && data.items.length === 0 && (
        <EmptyResults variant="empty" title="Aucun événement" text="Créez le premier événement." />
      )}
      ...
      <EditEventModal eventId={editId} onClose={() => setEditId(null)} />
      <ConfirmDialog
        open={!!deleteTarget}
